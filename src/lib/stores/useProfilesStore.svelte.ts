 ...
 * - Username:
 *   this.state.addresses.toBeCheckedFor.nostr.kind[0]
 * - Preferred relays:
 *   this.state.addresses.toBeCheckedFor.nostr.kind[10002]
 *
 ...
 *   this.state.addresses.toBeCheckedFor.nostr.kind[10002]
 * - Public messages:
 *   this.state.addresses.toBeCheckedFor.nostr.kind[1]
 *
 ...
 *   this.state.profiles[index].nostr.events.kind[0]
 *
...
class ProfilesStore {
  ...
  enableNostrNetworkFetchProfiles:
    parseEnvBool(config.enableNostrNetworkFetchProfiles, true),
  enableNostrNetworkFetchPreferredRelays:
    parseEnvBool(config.enableNostrNetworkFetchPreferredRelays, true),
  enableNostrNetworkFetchMessages:
    parseEnvBool(config.enableNostrNetworkFetchMessages, true),
  // Use Nostr network:
  enableNostrNetworkUsePreferredRelays:
    parseEnvBool(config.enableNostrNetworkUsePreferredRelays, true),
  // Display data from Nostr relays:
  enableNostrDisplayProfilesUsernames:
    parseEnvBool(config.enableNostrDisplayProfilesUsernames, true),
  enableNostrDisplayProfilesAbouts:
    parseEnvBool(config.enableNostrDisplayProfilesAbouts, true),
  enableNostrDisplayProfilesWebsites:
    parseEnvBool(config.enableNostrDisplayProfilesWebsites, true),
  enableNostrDisplayProfilesPictures:
    parseEnvBool(config.enableNostrDisplayProfilesPictures, true),
  enableNostrDisplayProfilesMessages:
    parseEnvBool(config.enableNostrDisplayProfilesMessages, true),
  // Actual state:
      ...
      nostr: {
        relays: {
          default: (
            ...
            typeof(config.nostrDefaultRelays) === "string"
            ? (
              config.nostrDefaultRelays.split(',')
  ...
  */
  get getMetadataByAddressNostr() {
    ...
    return (
      ...
      if (!this.state.enableNostrDisplayProfilesUsernames) {
        if (dataToExtract === "username") return ""
      }

      if (!this.state.enableNostrDisplayProfilesAbouts) {
        if (dataToExtract === "about") return ""
      }

      if (!this.state.enableNostrDisplayProfilesWebsites) {
        if (dataToExtract === "website") return ""
      }

      if (!this.state.enableNostrDisplayProfilesPictures) {
        if (dataToExtract === "picture") return ""
      }

      // Check if address is already linked with a profile.
      let addressProfile = this.state.profiles.find(
        (profile) => {
  ...
  get getProfilesWithPreferredRelays(): ProfileSpasm[] {
    const profilesWithPreferredRelays = this.state.profiles
                                        .filter((profile) => {
  ...
  get getAddressesWithPreferredRelays(): string[] {
  ...
  get getPreferredRelaysFromAddress() {
  ...
  get getMessagesByAddressNostr() {
    return (address: string): NostrEventSignedOpened[] => {
      const addressNpub = convertHexOrNpubAddressToNpub(address)

      // Find existing profile with the same id
      const profile: ProfileSpasm | undefined = this.state.profiles.find(
        profile => profile.id === addressNpub
      )
  ...
  addAddress(
    ...
    // console.log("env enableNostrNetwork:", this.state.enableNostrNetwork)
    // console.log("env enableNostrNetworkFetchProfiles:", this.state.enableNostrNetworkFetchProfiles)
    // console.log("env enableNostrNetworkFetchPreferredRelays:", this.state.enableNostrNetworkFetchPreferredRelays)
    // console.log("env enableNostrNetworkFetchMessages:", this.state.enableNostrNetworkFetchMessages)
    // console.log("env enableNostrNetworkUsePreferredRelays:", this.state.enableNostrNetworkUsePreferredRelays)
    // console.log("env enableNostrDisplayProfilesUsernames:", this.state.enableNostrDisplayProfilesUsernames)
    // console.log("env enableNostrDisplayProfilesPictures:", this.state.enableNostrDisplayProfilesPictures)
    // console.log("env enableNostrDisplayProfilesMessages:", this.state.enableNostrDisplayProfilesMessages)
    // console.log("env this.state.addresses.toBeCheckedFor.nostr.relays.default:", this.state.addresses.toBeCheckedFor.nostr.relays.default)
    ...
    if (
      ...
      // ENS
      if (!this.state.addresses.toBeCheckedFor.ethereum.ens.includes(address)) {
        ...
        if (
          ...
          this.state.addresses.toBeCheckedFor.ethereum.ens.push(address)
    ...
    if (
      ...
      if (!this.state.addresses.toBeCheckedFor.nostr.kind[0].includes(address)) {
        // Always add an address if forceUpdate is set
        if (
          updateType.includes("username") &&
          forceUpdate
        ) {
          this.state.addresses.toBeCheckedFor.nostr.kind[0].push(address)
      ...
      if (!this.state.addresses.toBeCheckedFor.nostr.kind[10002].includes(address)) {
        // Always add an address if forceUpdate is set
        if (
          updateType.includes("relays") &&
          forceUpdate
        ) {
          this.state.addresses.toBeCheckedFor.nostr.kind[10002].push(address)
      ...
      if (!this.state.addresses.toBeCheckedFor.nostr.kind[1].includes(address)) {
        // Always add an address if forceUpdate is set
        if (
          updateType.includes("messages") &&
          forceUpdate
        ) {
          this.state.addresses.toBeCheckedFor.nostr.kind[1].push(address)
  ...
  async updateAllProfiles(
    ...
    // console.log(
    //   time,
    //   "updateAllProfiles called for deepCopyOfObject(this.state.addresses):",
    //   deepCopyOfObject(this.state.addresses)
    // )
    ...
    if (this.state.isUpdatingProfiles) {
      // console.log("Updating profiles is already running. Aborting...")
      return
    }

    this.state.isUpdatingProfiles = true
     ...
     * change the original state queue (this.state.addresses).
    */
    const queue: Queue = deepCopyOfObject(
      this.state.addresses.toBeCheckedFor
    )
    ...
    if (
      ...
      Array.isArray(protocols) &&
      protocols.includes("nostr") &&
      this.state.enableNostrNetwork
    ) {
      this.updateAllProfilesNostr(queue, updateType, forceUpdate)
    }

    this.state.isUpdatingProfiles = false
  }
  ...
  async updateAllProfilesNostr(
    ...
    if (this.state.isUpdatingProfilesNostr) {
      // console.log("Updating profiles is already running for Nostr. Aborting...")
      return
    }

    this.state.isUpdatingProfilesNostr = true

    // Filter out update types which are disabled in .env
    if (!this.state.enableNostrNetworkFetchProfiles) {
      updateType = updateType.filter(type => type !== "username")
    }

    if (!this.state.enableNostrNetworkFetchPreferredRelays) {
      updateType = updateType.filter(type => type !== "relays")
    }

    if (!this.state.enableNostrNetworkFetchMessages) {
    ...
    //   deepCopyOfObject(this.state.addresses.toBeCheckedFor.nostr.kind)
    // )
    ...
    //   this.state.addresses.toBeCheckedFor.nostr.kind
    // ) as (
    ...
    kinds.forEach((kind) => {
      ...
      //   "addresses:",
      //   this.state.addresses.toBeCheckedFor.nostr.kind[kind])
    })
    ...
    if (this.state.enableNostrNetworkUsePreferredRelays) {
    ...
    // this.state.addresses.toBeCheckedFor.nostr.relays
    this.state.addresses.toBeCheckedFor.nostr.kind = deleteMatchingValuesFromObject(
      this.state.addresses.toBeCheckedFor.nostr.kind, queue.nostr.kind
    )


    this.state.isUpdatingProfilesNostr = false
    // console.log(time, "finished updating profiles")
    // console.log(
    //   "deep copy of this.state.addresses.toBeCheckedFor.nostr.kind[0] when updateAllProfilesNostr is finished:",
    //   deepCopyOfObject(this.state.addresses.toBeCheckedFor.nostr.kind[0])
    // )
  }
  ...
  markRelaysAsCheckedForAddressesForKinds(
    ...
    addresses.forEach((address) => {
      ...
      // Find existing profile with the same id
      let index = this.state.profiles.findIndex(
        profile => profile.id === addressNpub
      )
      ...
      kinds.forEach((kind) => {
        relays.forEach((relayUrl) => {
          pushToArrayIfValueIsUnique(
            this.state.profiles[index].nostr.relays.checkedRelays
            .checkedForEventsOfKind[kind],
            relayUrl
          )
  ...
  handleIncomingNostrEvent(
    ...
    // console.log("this.state.profiles after all handle:", this.state.profiles)
    // console.log("===============================")
  }
}

export const profilesStore = new ProfilesStore()
export const useProfilesStore = () => profilesStore
