 ...
 */
  ...
  const mergeObjects = (
    ...
    const mergedObject: Record<string, any> = { ...defaultObject };
  ...
  const parseEnvBool = (
    value: any, defaultValue: boolean = true
  ) => {
    if (typeof value === 'boolean') return value
    if (typeof value === 'string') {
      if (value === 'true') return true
      if (value === 'false') return false
      // value can also be an empty string, so in that case
      // we should skip and return defaultValue
    }
    return defaultValue
  }
  ...
  const logExecution = (
    ...
    // use console (will appear in server logs for SSR or browser console for client)
    if (logType === 'log') {
      console.log(`[${when}] ${val} executed on: ${side}`)
    } else if (logType === 'error') {
      console.error(`[${when}] ${val} executed on: ${side}`)
