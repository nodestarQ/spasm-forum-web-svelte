...
class EventsStore {
  state: PostsState = $state({
    ...
    useMockedDataIfBackendIsDown: config.useMockedDataIfBackendIsDown === "true" ? true : false,
    ...
    // Short IDs
    enableShortUrlsForWeb3Actions: config.enableShortUrlsForWeb3Actions === "true" ? true : false,
  ...
  get getPinnedPosts(): SpasmEventV2[] {
      if (isArrayWithValues(this.state.pinnedIds)) {
        return spasm.getEventsByIds(this.state.allPosts, standardizeIds(this.state.pinnedIds)) || []
      } else { return [] } 
    }

  get getPosts(): SpasmEventV2[] {
      const filteredEvents = this.filterEvents(this.state.displayFilters)
  ...
  get getPost(): SpasmEventV2 | null {
      if (
        !this.state.currentEventId ||
        !isStringOrNumber(this.state.currentEventId)
      ) { return null }

      const id = this.state.currentEventId.toString()
      let event: SpasmEventV2 | null = null

      if (
        this.state.enableShortUrlsForWeb3Actions &&
        this.state.shortUrlsLengthOfWeb3Ids &&
        typeof(this.state.shortUrlsLengthOfWeb3Ids) === "number"
      ) {
        event = spasm.getEventById(
          this.state.allPosts, id, this.state.shortUrlsLengthOfWeb3Ids
        )
      } else {
        event = spasm.getEventById(this.state.allPosts, id)
      }
  ...
  updateStateAppConfig(): string {
      try {
        ...
        this.state.pinnedIds = appConfig?.pinnedIds || []
    ...
    async fetchPostsByFilters(
      ...
      if (standardizeIds(this.state.pinnedIds)) {
        await this.fetchAndSaveEventsByIds(
          standardizeIds(this.state.pinnedIds)
        )
      }
      ...
      this.state.fetchingPostsByFilters = true
      ...
      if (
        this.state.useMockedDataIfBackendIsDown &&
        !isArrayWithValues(fetchedEvents.value)
      ) {
      ...
      this.state.fetchingPostsByFilters = false
    ...
    async fetchRepliesFromAllNetworksByIds(
      ...
      try {
        ...
        if (
          ...
          this.addEventsToTreeByIds(
            destinationTreeIds || null, spasmEvents
          )
        }
    ...
    addEventsToTreeByIds(
      ...
      try {
        ...
        if (!isArrayWithValues(this.state.allPosts)) return

        this.state.allPosts.forEach((event: SpasmEventV2) => {
          let ifMatch = false
          treeIds.forEach(id => {
    ...
    saveEventsToStore(events: SpasmEventV2[] | null): boolean {
      ...
      const concatEvents = events.concat(this.state.allPosts)
      ...
      this.state.allPosts = sortedEvents
      ...
      if (browser) {
        const profilesStore = useProfilesStore()
        profilesStore.addAddressesFromSpasmEvents(this.state.allPosts)
      }
    ...
    filterEvents(
      ...
      spasmEvents = spasmEvents || this.state.allPosts || []
      ...
      const filterByActivity = (
        ...
        if (filters.activity === "hot") {
          value = this.state.feedFiltersActivityHot || 5
        } else if (filters.activity === "rising") {
          value = this.state.feedFiltersActivityRising || 3
        } else {
          return true
        }
      ...
      if (selectedIdsNotDisplayed?.length > 0) {
        const foundEvents: SpasmEventV2[] | null =
          ...
          this.state.allPosts, selectedIdsNotDisplayed
        )
      ...
      if (isArrayWithValues(standardizeIds(this.state.pinnedIds))) {
        standardizeIds(this.state.pinnedIds).forEach(pinnedId => {
    ...
    setSelectedIds(
      ...
      this.state.selectedIds = arrayOfIds
    }
    ...
    setCurrentPostId(id: (string | number)): void {
      ...
      this.state.currentEventId = newId
    }

    async updateCurrentPost():Promise<void> {
      if (this.state.currentEventId) {
        await this.fetchAndSaveEventsByIds([this.state.currentEventId])
      }
    }
    ...
    getPostsByShortId(
      ...
      try {
        ...
        this.state.allPosts.forEach(event => {
    ...
    // ): Promise<SpasmEventV2[] | { error: string }[]> {
      ...
      const eventsFromStore: SpasmEventV2[] | null =
        spasm.getEventsByIds(this.state.allPosts, arrayOfIds)
